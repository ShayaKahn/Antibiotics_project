...
class IDOA:
    ...
    cohort.
    """
    def __init__(self, first_cohort, second_cohort, identical=False, min_overlap=0.5, max_overlap=1,
                 zero_overlap=0.1, pos=True):
        """
        ...
        if self.identical:
            for i in range(0, self.num_samples_second):
                ...
                if np.max(overlap_vector) <= self.min_overlap + self.zero_overlap:
                    ...
                    self.dissimilarity_overlap_container.append(np.vstack((new_overlap_vector,
                                                                          new_dissimilarity_vector)))
    ...
    def calc_idoa_vector_custom(self, second_cohort_ind_dict):
        for i in range(0, self.num_samples_second):
            o_vector = []
            d_vector = []
            for j in range(0, self.num_samples_first):
            ...
            if i in second_cohort_ind_dict:
                ...
                overlap_vector = np.array([o_vector[j].calculate_overlap()
                                           for j in range(0, self.num_samples_first) if j != k])
                dissimilarity_vector = np.array([d_vector[j].calculate_dissimilarity()
                                                 for j in range(0, self.num_samples_first) if j != k])
            else:
                overlap_vector = np.array([o_vector[j].calculate_overlap()
